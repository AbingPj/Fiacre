			...
			<div class="col-6">
				<h3>
					...
					<download-excel
						...
						:header="
							'Daily Sales Report (' + dateLabel.replace(/,/g, '.') + ')'
						"
					...
					<table class="table table-hover">
						...
						<tbody>
							<tr v-if="data.length == 0">
								<td colspan="5" class="bg-secondary text-center">
									No Orders yet for {{ dateLabel }}
								</td>
							</tr>
							<tr v-for="(item, index) in data" :key="index">
								...
								<td>
									<span class="float-right">{{
										displayNumberWithComma(item.total_earnings)
									}}</span>
								</td>
								<td>
									<a
						...
						<tfoot>
							<tr>
								...
								<th>
									<span class="float-right"
										>$ {{ displayNumberWithComma(total) }}</span
									>
								</th>
								<th></th>
							</tr>
	...
	export default {
		...
		methods: {
			...
			getOrganization() {
				...
				axios
					.get("/admin/api/data/getOrganization")
					.then((res) => {
