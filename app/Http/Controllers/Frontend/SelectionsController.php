...
class SelectionsController extends Controller
{
    ...
    {
        $search = $request->search;
        $data = Organization::select('org_name','org_optionc_id','org_cityprov','org_zipcode','org_state')
            ->where('org_name', 'LIKE', "%$search%")
            ->orWhere('org_optionc_id', 'LIKE', "%$search%")
            ->orWhere('org_cityprov', 'LIKE', "%$search%")
            ->orWhere('org_zipcode', 'LIKE', "%$search%")
            ->orWhere('org_state', 'LIKE', "%$search%")
            ->orderBy('org_name')
            ->skip(0)
            ->take(50)
            ->get()
